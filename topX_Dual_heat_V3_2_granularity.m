...
% A specific version for material interpolate
% Give up using robust TO since there is no confirmed relation between robust and heat targets

% Example  [xPhys_cat, c, Q] = topX_Dual_heat_V3_2_granularity(5,1,200,0.16,31,0.5,1,0,1,2000,0.0001,1,1,'test_left.png','test_right.png');
function [xPhys_cat, c, Q] = topX_Dual_heat_V3_2_granularity(CellCount,CELL_TYPE,nel,Kmin,Kmax,w1,GLOBAL,LOCAL,FilterDom,nloop,epsilon,IC,assign,left_pic,right_pic)
...
if CELL_TYPE==1
    sym = 4;
    if assign
        left_struc=imread(left_pic);
        left_struc=1-left_struc;
        right_struc=imread(right_pic);
        right_struc=1-right_struc;
        left_tensor=heat_microstructure_homogenize(nelx,nely,Kmax,Kmin,left_struc);
        right_tensor=heat_microstructure_homogenize(nelx,nely,Kmax,Kmin,right_struc);
        kalpafrac_a=linspace(left_tensor(1,1),right_tensor(1,1),CellCount);
        kalpafrac_b=linspace(left_tensor(2,2),right_tensor(2,2),CellCount);
        for i=1:CellCount
...
%modification for interpolation
if assign
    x{1}=left_struc;xTilde{1}=left_struc;xPhys_i{1}=left_struc;xold1{1}=left_struc(:);
    x{CellCount}=right_struc;xTilde{CellCount}=right_struc;xPhys_i{CellCount}=right_struc;xold1{CellCount}=right_struc(:);
end
...
while (change > 0.0001) && loop<nloop
   ...
   %Objectives for intercells: minimize interccell heat damps
    ...
    for i = 1:CellCount
        ...
        %modifcation for interpolation
        if assign && i==1
            x{i}=left_struc;
            xPhys_i{i}=left_struc;
            xTilde{i}=left_struc;
        elseif assign && i==CellCount
            x{i}=right_struc;
            xPhys_i{i}=right_struc;
            xTilde{i}=right_struc;
        end
    end
