...
% V4.0: Add vertical compabilities
% Give up using robust TO since there is no confirmed relation between robust and heat targets

% Example  [xPhys_cat, c, Q] = topX_Dual_heat_V2(3,1,100,0.16,31,0.5,1,0,1,2000,0.0001,1,1,[10,10],[20,20],0);

function [xPhys_cat, c, Q] = topX_Dual_heat_V4(CellCount,CELL_TYPE,nel,Kmin,Kmax,w1,GLOBAL,LOCAL,FilterDom,nloop,epsilon,IC,assign,left,right,vertical)
...
if vertical
    nodenrs_vertical = cell(2,1);
    edofMat_vertical = cell(2,1);
    iK_vertical = cell(2,1);
    jK_vertical = cell(2,1);
    [nodenrs_vertical{1}, edofMat_vertical{1}, iK_vertical{1}, jK_vertical{1}] = PrepareFEA(nelx,nely);
    [nodenrs_vertical{2}, edofMat_vertical{2}, iK_vertical{2}, jK_vertical{2}] = PrepareFEA(nelx,2*nely);
end
...
wfixed = cell(2,1);
if vertical
    T_i_vertical = cell(2,1);
    Tfixed_vertical = cell(2,1);
    d1_vertical = cell(2,1);
    d2_vertical = cell(2,1);
    d3_vertical = cell(2,1);
    d4_vertical = cell(2,1);
    wfixed_vertical = cell(2,1);
end
...
if vertical
    [T_i_vertical{1}, Tfixed_vertical{1}, d1_vertical{1}, d2_vertical{1}, d3_vertical{1}, d4_vertical{1}, wfixed_vertical{1}] = ApplyPeriodicBC(nelx,nely,nodenrs_vertical{1,1});
    [T_i_vertical{2}, Tfixed_vertical{2}, d1_vertical{2}, d2_vertical{2}, d3_vertical{2}, d4_vertical{2}, wfixed_vertical{2}] = ApplyPeriodicBC(nelx,2*nely,nodenrs_vertical{2,1});
end
...
if vertical
    Q_i_vertical = cell(CaseCount,1);
    dQ_i_vertical = cell(CaseCount,1);
    for i = 1:CaseCount
        Q_i_vertical{i,1} = zeros(2,2);
        dQ_i_vertical{i,1} = cell(2,2);
    end
end
...
%modification: add up vertical combinations
if vertical
    xTilde_vertical = cell(CaseCount,1);
    xPhys_i_vertical = cell(CaseCount,1);
    for i=1:CellCount
        xTilde_vertical{i}=xTilde{i};
        xPhys_i_vertical{i}=xPhys_i{i};
    end
    ...
    while i <= CaseCount
        ...
        k = j + l;
        while k <= CellCount
            xTilde_vertical{i} = vertcat(xTilde_vertical{j},xTilde_vertical{k});
            xPhys_i_vertical{i} = vertcat(xPhys_i_vertical{j},xPhys_i_vertical{k});
...
if vertical
    c_hist_vertical = zeros(nloop,CaseCount);        % objective
end
%% START ITERATION
while (change > 0.0001) && loop<nloop
    ...
    if vertical
        for i = 1:CaseCount
            if i <= CellCount
                T_i_vertical{i} = FEAnalysis(nelx,nely,KE,Kmin,xPhys_i_vertical{i},penal,Kmax,iK_vertical{1},jK_vertical{1},d1_vertical{1},d2_vertical{1},d3_vertical{1},d4_vertical{1},Tfixed_vertical{1},wfixed_vertical{1});
                [Q_i_vertical{i}, dQ_i_vertical{i}] = Homogenization(nelx,nely,T_i_vertical{i},edofMat_vertical{1},Kmin,Kmax,xPhys_i_vertical{i},penal,KE);
            else
                T_i_vertical{i} = FEAnalysis(nelx,2*nely,KE,Kmin,xPhys_i_vertical{i},penal,Kmax,iK_vertical{2},jK_vertical{2},d1_vertical{2},d2_vertical{2},d3_vertical{2},d4_vertical{2},Tfixed_vertical{2},wfixed_vertical{2});
                [Q_i_vertical{i}, dQ_i_vertical{i}] = Homogenization(nelx,2*nely,T_i_vertical{i},edofMat_vertical{2},Kmin,Kmax,xPhys_i_vertical{i},penal,KE);
            end
        end
    end
   ...
   %Objectives for intercells: minimize interccell heat damps
    ...
    %modification: add vertical objects
    if vertical
        i = CellCount + 1;
        l = 1;
        while i <= CaseCount
            ...
            while k <= CellCount
                ...
                [c_i_temp,dc_i_temp] = Objective(Q_i_vertical{i},dQ_i_vertical{i},3,3);
                
                c_hist_vertical(loop,i) = c_i_temp;
                
                c_i = c_i + w * c_i_temp;
                current_heatdamps = current_heatdamps+c_i_temp;
                tmp_i = + w * dc_i_temp;
                tmp_i_U = tmp_i(1:nely,1:nelx);
                tmp_i_D = tmp_i((nely+1):end,1:nelx);
                dc_i(1:nely,(((j-1)*nelx)+1):(j*nelx)) = dc_i(1:nely,(((j-1)*nelx)+1):(j*nelx)) + tmp_i_U;
                dc_i(1:nely,(((k-1)*nelx)+1):(k*nelx)) = dc_i(1:nely,(((k-1)*nelx)+1):(k*nelx)) + tmp_i_D;
    ...
    if LOCAL == 1
        for i = 1:CellCount
            fval(M*CellCount+i) = 1000*((sum(x_pde_hat_i{i}.^p))^(1/p)- vol_max_pNorm(i));
            dfdx(M*CellCount+i,((i-1)*(nelx*nely)+1):(i*(nelx*nely))) = 1000*TF'*(LF'\(LF\(TF*dfdx_pde_i{i}(:))));
    ...
    if Influence == 1
        ...
        if vertical
            ...
            hold on
            for i = CellCount+1:CaseCount
                plot(c_hist_vertical(1:loop,i),'g')
            end
            hold off
            title('Heat Damps Vertical');
        end
